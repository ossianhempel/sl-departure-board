...
1. **Start with convex-helpers** - Don't reinvent common patterns
2. **Custom Functions for Auth** - Create `authedQuery`, `authedMutation`, etc.
3. **Relationships Over Nesting** - Use relationship helpers, keep data normalized
4. **Filter Sparingly** - Prefer indexes when possible
5. **Sessions for Anonymous Users** - Track before signup, migrate to user account later
